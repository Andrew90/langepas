...
void CommonViewer::operator()(TMouseWell &l)
{
		...
		if(0 == l.flags.lButton)
		{
			...
			else  if(currentX < chart->minAxesX) 	currentX = (int)chart->minAxesX;
		}
		else
		{
			currentY -= offs;
		
			if(currentY >= chart->maxAxesY) 	currentY = count_sensors - 1;
			else  if(currentY < chart->minAxesY) 	currentY = (int)chart->minAxesX;
		}
...
LRESULT CommonViewer::operator()(TCreate &l)
{
	...
	chart->minAxesY = 1;
	chart->maxAxesY = 1 + count_sensors;
	chart->minAxesX = 0;
