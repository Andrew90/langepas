...
#define L502_DEVNAME_SIZE     LPCIE_DEVNAME_SIZE
/** Максимальное количество символов в строке с серийным номером */
#define L502_SERIAL_SIZE      LPCIE_SERIAL_SIZE
...
typedef enum
{
    ...
    L502_GETDEVS_FLAGS_ONLY_NOT_OPENED = LPCIE_GETDEVS_FLAGS_ONLY_NOT_OPENED
...
typedef enum
{
    ...
    L502_DEVFLAGS_DAC_PRESENT         = LPCIE_DEVINFO_FLAGS_DAC_PRESENT,
    /** Признак наличия гальваноразвязки */
    L502_DEVFLAGS_GAL_PRESENT         = LPCIE_DEVINFO_FLAGS_GAL_PRESENT,
    /** Признак наличия сигнального процессора BlackFin */
    L502_DEVFLAGS_BF_PRESENT          = LPCIE_DEVINFO_FLAGS_BF_PRESENT,    
...
typedef enum
{
    ...
    L502_OUT_CYCLE_FLAGS_FORCE = 0x01
...
typedef struct st_l502* t_l502_hnd;
...
typedef char (*t_l502_serial_list)[L502_SERIAL_SIZE];
...
/***************************************************************************//**
    ...
    передать в функцию флаг #L502_GETDEVS_FLAGS_ONLY_NOT_OPENED.

 @param[in] serials   Массив размером size*#L502_SERIAL_SIZE байт, в который
 ...
 @param[in] flags     Флаги из #t_l502_getdevs_flags, определяющие поведение
                      функции.
...
/***************************************************************************//**
    @brief Открытие модуля по его серийному номеру.
    ...
    L502_Close() никто другой установить связь с модулем не сможет
    (будет возвращена ошибка #L502_ERR_DEVICE_ACCESS_DENIED).
    ...
    #L502_ERR_DEVICE_NOT_FOUND. Если в системе присутствуют модули L502, но
...
/***************************************************************************//**
    @brief Открытие устройства по структуре с информацией о нем.

    Открытие устройства L502, соответствующего заданному элементу из массива со
    структурами, содержащими информацию о подключенных устройствах,
    поддерживаемых драйвером lpcie. Этот массив структур может быть получен
    вызовом функции LPCIE_GetDevInfoList().
    @warning Эта функция может быть изменена в последующих версиях библиотеки
             и пока не рекомендуется к использованию.

    @param[in] hnd     Описатель устройства, созданный с помощью L502_Create()
    @param[in] info    Структура с информацией об устройстве
    @return            Код ошибки
*******************************************************************************/
LPCIE_EXPORT(int32_t) L502_OpenByListItem(t_l502_hnd hnd, t_lpcie_devinfo* info);
...
/***************************************************************************//**
    ...
    Пользовательские команды начинаются с кода L502_BF_CMD_CODE_USER (0x80000000).
